...
/*  A thread group contains configuration information and can run its "gang" */
/*  of threads performing a particular mix of operations */

/* The thread group is responsible for creating the ffsb_thread objects */
/* which must ask the thread group object to select an operation and  */
/* a filesystem to run that operation on */
/* The thread objects don't contain any of the configuration information */

/* Thread groups are also abstracted so they can be "run" arbitrarily */
/* which is useful because we can reuse them for aging */
/* the running is a bit complex, the thread group has to have a callback */
/* function which is runs at a specified interval to determine when to */
/* terminate */
...
/* synchronize multiple thread-groups being ready to start, meaning that */
/* all their threads have been spawned */
/* The second one "thread_barrier" synchronizes all threads across multiple */
/* thread groups, so that they all start at once */
...
/* parameters needed to fire off a thread group */
/* the main thread will evaluate poll_fn( poll_data) */
/* until it gets a nonzero return value */
/* it will sleep for wait_time secs between calls */
/* The ffsb_config struct is needed for fs selection */
/* barriers are to synchronize multiple tgs and all threads */
/* pt is for pthread_create() */
...
/* this function is meant to be called as a parameter to pthread_create() */
void*      tg_run( void *  );
...
/* the threads in the tg should be the only ones using these (below) funcs */
...
/* The threads in this tg will use this function to get an op*/
/* to run, so all configuration specific information is kept */
/* in this object */
typedef struct tg_op_params {
/* 	struct ffsb_config *fc; */ /* out parameter */
