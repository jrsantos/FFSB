...
{
  /* Recursively determine the depth of the left and right subtrees */
...
rb_node * rbtree_insert(rb_tree * tree, datatype object)
{
  ...
  if (!(tree->root)) {
    /* Assign a new root node (the root is always black) */
  ...
  /* Find a spot for the new object, insert the object as a red leaf */
  /* *************************************************************** */
    ...
    /* Compare inserted object with the object stored in the current node */
	  ...
	  if ( comp_result > 0) {
		  if (!(cur_node->left)) {
			  ...
			  cur_node = cur_node->left; /* Go to the left subtree */
		  }
	  } else {
		  if (!(cur_node->right)) {
			  ...
			  cur_node = cur_node->right; /* Go to the right subtree */
		  }
	  }
  }
...
rb_node * insert_successor_at(rb_tree * tree,
                                     rb_node * at_node, datatype object)
{
  rb_node * parent;
  rb_node * new_node;
  ...
  if (!at_node) {
    /* The new node should become the tree's minimum          */ 
    /* Place is as the left child of the current minimal leaf */
...
rb_node * insert_predecessor_at(rb_tree * tree,
                                       rb_node * at_node, datatype object)
{
  rb_node * parent;
  rb_node * new_node;
...
void rbtree_remove_at(rb_tree * tree, rb_node * node, destructor d)
{
 ...
 /* to delete it.                                                           */
  ...
  /* Splice out the node to be removed, by linking its parent straight to */ 
  /* the removed node's single child.                                     */
  ...
  /* Delete the un-necessary node (nullify both its children because the    */
...
void rbtree_insert_fixup(rb_tree * tree, rb_node * node)
{
  ...
  while (curr_node != tree->root && curr_node->parent->color == red) {
    /* Get a pointer to the current node's grandparent (the root is */
    ...
    if (curr_node->parent == grandparent->left) {
      ...
      /* grandparent red. In case of a NULL uncle, treat it as a black node */
...
void rbtree_remove_fixup(rb_tree * tree, rb_node * node)
{
  ...
  while (curr_node != tree->root && curr_node->color == black) {
	...
	/* In case the sibling is red, color it black and rotate.      */
        /* Then color the parent red (the grandparent is now black)    */
