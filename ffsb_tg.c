...
static 
void update_bufsize(ffsb_tg_t *tg)
{
...
static
void       tg_print_config_helper(ffsb_tg_t *tg)
{
	int i;
	int sumweights = 0;
	char buf[256];
	...
	for( i = 0; i < FFSB_NUMOPS; i++) {
		printf("\t %20s = %d (%.2f%%)\n",op_get_name(i),tg->op_weights[i],
		       100 * (float)tg->op_weights[i] /(float)sumweights );
	}	
	printf("\t\n");
}
